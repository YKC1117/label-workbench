...
const SAMPLES=[
  ...
  ['officialCEA','https://www.bartendersoftware.com/download-resource?resourceId=79797'],
...
function headerText(data,limit){return data.subarray(0,limit).toString('latin1').replace(/\0/g,'')}
...
async function inspect(name,url){
  const res=await fetch(url,{redirect:'follow'});assert(res.ok,`${name} download ${res.status}`);const data=Buffer.from(await res.arrayBuffer()),parsed=parse(data),strings=scanStrings(parsed.container),tags=scanAsciiTags(parsed.container),header=headerText(data,parsed.metaEnd);
  console.log(`=== ${name} === bytes=${data.length} container=${parsed.container.length} tags=${tags.length}`);
  console.log('HEADER',JSON.stringify(header.match(/Application:[^\r\n]+|Document:[^\r\n]+/g)||[]));
  const design=tags.filter(t=>/^(?:TextData|BcC128Data|BcDatamatrixData|PictureData|LineData|BoxData|ShapeData)$/.test(t.text));
  console.log('DESIGN_OBJECTS',JSON.stringify(design.map(t=>({offset:t.offset,type:t.text}))));
  for(const t of tags)console.log(`TAG ${t.offset} ${t.text}`);
  const template=strings.find(s=>s.text==='Template 1'||/^Template \d+$/.test(s.text));if(template){boundaryProbe(parsed.container,'Template',template.offset,48,80);const roots=strings.filter(s=>s.offset>template.offset&&s.offset<template.offset+30000&&/^Root\./.test(s.text));for(const r of roots.slice(0,30))boundaryProbe(parsed.container,`ROOT:${r.text}`,r.offset,96,24)}
  if(name==='officialCEA'){
    const interesting=strings.filter(s=>s.text.length<=160&&!/^Root\./.test(s.text)&&!/^<.*>/.test(s.text)&&!/(Arial|Tahoma|Segoe|Times|Courier)/i.test(s.text));
    console.log('CEA_STRINGS');for(const s of interesting.slice(0,260))console.log(`${s.offset}\t${JSON.stringify(s.text)}`);
  }
  for(const t of design)boundaryProbe(parsed.container,`TAG:${t.text}`,t.offset,48,48);
