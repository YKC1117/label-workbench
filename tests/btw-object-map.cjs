...
const S=[
 ['c128','https://raw.githubusercontent.com/Seagull-Scientific/bartender-cloud-api/main/Sample_Doc1.btw','BcC128Data'],
 ['dm','https://support.seagullsoftware.com/hc/en-us/article_attachments/360011164514','BcDatamatrixData']
];
...
(async()=>{for(const [name,url,want] of S){const r=await fetch(url);const c=parse(Buffer.from(await r.arrayBuffer())),ts=tags(c),idx=ts.findIndex(t=>t.type===want),t=ts[idx],next=ts.slice(idx+1).find(x=>/^(?:TextData|Bc|PictureData|BackgroundData)/.test(x.type));const end=next?.offset||c.length;console.log(`===${name} ${want} ${t.offset}-${end} len=${end-t.offset}===`);for(const s of strings(c,t.offset,end))console.log(`${s.offset}\t${JSON.stringify(s.text)}`)}})().catch(e=>{console.error(e);process.exit(1)});
