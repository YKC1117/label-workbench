...
function typeTag(type){const b=new TextEncoder().encode(type),h=new Uint8Array([0xff,0xff,0x01,0x00,b.length&255,(b.length>>>8)&255]);return concat([h,b])}
...
if(englishBarcode.owner!=='')throw new Error(`stale owner leaked across objects: ${englishBarcode.owner}`);
if(lastText?.value!=='3')throw new Error(`last object swallowed document tail: ${lastText?.value}`);
...
if(last?.owner!=='')throw new Error(`stale owner returned after variable-length edits: ${last?.owner}`);
if(finalText?.value!=='3')throw new Error(`document tail contaminated last text after edits: ${finalText?.value}`);
...
console.log('PASS: last-record boundary remains stable after variable-length edits');
