...
for marker in ['assets/ui-refresh.css?v=20260910-v190','assets/nav-groups.css?v=20260910-v110','assets/app.js?v=20260910-v181','assets/cloud.js?v=20260910-v192']:
    if marker not in html: raise SystemExit(f'index.html missing current cache marker: {marker}')
for marker in ['assets/cloud-attachments.js','assets/file-parsers.js','assets/barcode-reader.js','20260910-v192']:
    if marker not in cloud: raise SystemExit(f'cloud loader missing: {marker}')
for marker in ['barcode-reader-core.js','barcode-reader-ui.js','barcode-generator.js','label-interpreter.js','bt-quick.js','bt-bridge.js','workbench-priority.js','20260910-v192']:
...
# BT Quick Production: data pack + resilient module registration/recovery
for marker in ['20260910-bt130','jszip@3.10.1','labelWorkbench.btDraft.v1','BT_Data.csv','BT_Field_Map.csv','BT_Barcode_Map.csv','BT_WorkPack.json','BT_Open.cmd','buildOpenCmd','templateBaseName','inferFieldUsage','mapBarcodes','normalizeDraft','safeRender','window.LabelWorkbenchBtQuick=API','本批有變動','本批固定','/DbTextHeader=1']:
...
for marker in ['20260910-btb130','analysisSendBt','receiveAnalysis','建立 BT 製作包（自動下載）','wireInterpreter','wireParsers','parseTableFiles','tableResult',"['csv','xls','xlsx']",'downloadProductionPack','ensureBtQuick','bt130-retry','BT_製作包_','BT_Data.csv','ZIP 建立失敗，已改下載 BT_Data.csv']:
...
print('PASS: v1.9.2 cache/load chain and BT-first navigation checked')
