...
async function verify(label,expectedSeed,expectedKind){
  const R=c.LabelWorkbenchBtwRichNative,F=c.LabelWorkbenchBtwFormat,M=c.LabelWorkbenchBtwObjectMap;
  ...
  const parsed=F.parseStructure(out.bytes),container=await F.inflateContainer(parsed),map=M.mapContainer(container),active=activeObjects(M,container);
  assert(/^2022\b/i.test(String(parsed.header.applicationVersion||'')),`not a 2022 BTW: ${parsed.header.applicationVersion}`);
  ...
  console.log('PASS rich',expectedKind,JSON.stringify({seed:out.seed,version:parsed.header.applicationVersion,target:out.layout.target.raw,texts:out.layout.fields.length,barcodes:out.layout.barcodes.length,active:active.length,bytes:out.bytes.byteLength}));
}

(async()=>{
  const N=c.LabelWorkbenchBtwNative,R=c.LabelWorkbenchBtwRichNative,B=c.LabelWorkbenchBtwRichBridge;
  assert(R?.BUILD==='20260911-btw-rich-100','unexpected rich generator build');
