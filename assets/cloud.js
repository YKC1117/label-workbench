...
(function(){
  ...
  function setStatus(kind, text){
    ...
    const label = el('cloudStatusText');
  ...
  function injectPanel(){
    ...
    panel.innerHTML = `
      <div class="section-title">
        <h3>☁️ 跨裝置同步</h3>
        <div class="cloud-state"><span id="cloudDot" class="cloud-dot off"></span><span id="cloudStatusText">尚未設定</span></div>
      </div>
      <p class="muted cloud-copy">採「本機優先」設計：就算雲端或網路暫時故障，案件建立、編輯、條碼工具與 BarTender 前置工作仍可正常使用。</p>
      <div id="cloudSetupBox" class="note warn-note">
        雲端後端尚未啟用。完成 Supabase 專案與安全規則後，這裡會自動切換成登入與同步控制。
      </div>
      <div id="cloudAuthBox" class="cloud-auth hidden">
        <div class="field cloud-email-field">
          <label for="cloudEmail">登入 Email</label>
          <input id="cloudEmail" type="email" autocomplete="email" placeholder="輸入你的 Email">
        </div>
        <div class="toolbar cloud-buttons">
          <button id="cloudLoginBtn" class="btn primary" type="button">寄送登入連結</button>
          <button id="cloudSyncBtn" class="btn ghost hidden" type="button">立即同步</button>
          <button id="cloudLogoutBtn" class="btn ghost hidden" type="button">登出</button>
        </div>
        <div id="cloudMessage" class="footer-note"></div>
      </div>`;
    host.appendChild(panel);
  }

  function renderAuth(){
    const setup = el('cloudSetupBox');
    const auth = el('cloudAuthBox');
    const sync = el('cloudSyncBtn');
    const logout = el('cloudLogoutBtn');
    const login = el('cloudLoginBtn');
    const email = el('cloudEmail');
    if(!cloudReady()){
    ...
    if(state.session){
      if(email){ email.value = userEmail; email.disabled = true; }
      login?.classList.add('hidden');
      sync?.classList.remove('hidden');
      logout?.classList.remove('hidden');
      setStatus('ok', state.syncing ? '同步中' : '已登入');
      setText('cloudMessage', state.lastSync ? `最後同步：${new Date(state.lastSync).toLocaleString()}` : `已登入：${userEmail}`);
    }else{
      ...
      setStatus('idle','等待登入');
      setText('cloudMessage','使用 Email 登入後，公司電腦、家裡電腦、手機和平板可接續同一批案件。');
    }
  }

  async function sendMagicLink(){
    const email = (el('cloudEmail')?.value || '').trim();
    if(!email){ setText('cloudMessage','請先輸入 Email。'); return; }
    ...
    try{
      ...
      setText('cloudMessage','登入連結寄送失敗：' + describeError(err));
    }
  }
  ...
  function mergeCases(localCases, remoteRows){
    ...
    (remoteRows || []).forEach(row => {
      ...
      if(!remote.id) remote.id = row.case_id;
      const local = map.get(remote.id);
      if(!local || safeDate(remote.updatedAt || row.updated_at) > safeDate(local.updatedAt)) map.set(remote.id, remote);
    });
  ...
  function bind(){
    el('cloudLoginBtn')?.addEventListener('click', sendMagicLink);
    el('cloudSyncBtn')?.addEventListener('click', () => syncNow());
    el('cloudLogoutBtn')?.addEventListener('click', signOut);
