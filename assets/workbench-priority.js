...
(function(){
  ...
  const headerCopy={
    ...
    analysis:['快速分析','把客戶 PDF／圖片整理成可回覆、可製作、可確認的內容。'],
    bartender:['BT 快速製作','正式工作先下載 BT 可直接匯入圖檔；可編輯 BTW 保留實驗，不會塞測試資料。'],
  ...
  function injectUiRefresh(){
    ...
    link.rel='stylesheet';
    link.href=`assets/ui-refresh.css?v=${BUILD}`;
    link.dataset.lwUiRefresh='true';
  ...
  function bindHeaderCopy(){
    document.querySelectorAll('[data-view]').forEach(btn=>{
      ...
      btn.addEventListener('click',()=>setTimeout(()=>{setVersion();syncHeaderCopy();},0));
    });
  }
  ...
  function waitFor(getter,timeout=10000){
    ...
    return new Promise(resolve=>{const tick=()=>{const value=getter();if(value)return resolve(value);if(Date.now()-start>=timeout)return resolve(null);setTimeout(tick,80)};tick()});
  }
  ...
  async function runQuickAnalysis(files){
    ...
    try{
      ...
      out.innerHTML=`<div class="note warn-note"><b>快速分析失敗：</b>${esc(err?.message||err)}<br>請重新選擇檔案再試一次；若內容本身不足，系統會列出同事需要向客戶補確認的資料。</div>`;
    }finally{
      setVersion();
    }
  }
  ...
  function updateCopy(){
    ...
    const drop=analysis?.querySelector('.drop > p');
    if(drop) drop.textContent='PDF、圖片、Excel、Word、CSV 直接丟進來，整理成可回覆、可製作、可確認的內容。';
    const note=analysis?.querySelector('.warn-note');
    if(note) note.innerHTML='<b>快速分析：</b>PDF／圖片會自動判斷方向、分標籤、讀欄位位置、補讀小字並交叉比對條碼；畫面只留下可製作內容與待確認項目。';
