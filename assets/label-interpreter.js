...
(function(){
  ...
  function crop(src,x,y,w,h){
    ...
    const z=c.getContext('2d',{willReadFrequently:true});z.fillStyle='#fff';z.fillRect(0,0,c.width,c.height);z.drawImage(src,x,y,w,h,0,0,c.width,c.height);return c;
  }
  ...
  async function imageCanvas(file){
    ...
    const max=Math.max(img.naturalWidth||1,img.naturalHeight||1),scale=Math.max(1,Math.min(3,4200/max));
  ...
  function contentBounds(canvas,threshold=242){
    ...
    const sx=Math.max(1,Math.ceil(canvas.width/1800)),sy=Math.max(1,Math.ceil(canvas.height/1800));
    ...
    const px=Math.round(canvas.width*.018),py=Math.round(canvas.height*.018);
    minX=Math.max(0,minX-px);minY=Math.max(0,minY-py);maxX=Math.min(canvas.width-1,maxX+px);maxY=Math.min(canvas.height-1,maxY+py);
  ...
  function enhanceCanvas(src,mode='gray'){
    const trimmed=trimCanvas(src),target=Math.min(3600,Math.max(trimmed.width,2200)),scale=Math.max(1,Math.min(2.2,target/Math.max(1,trimmed.width)));
    const c=document.createElement('canvas');c.width=Math.round(trimmed.width*scale);c.height=Math.round(trimmed.height*scale);
    const x=c.getContext('2d',{willReadFrequently:true});x.imageSmoothingEnabled=true;x.drawImage(trimmed,0,0,c.width,c.height);
    const im=x.getImageData(0,0,c.width,c.height),d=im.data;let lo=255,hi=0;
    for(let i=0;i<d.length;i+=16){const g=grayAt(d,i);if(g<lo)lo=g;if(g>hi)hi=g}
    if(hi-lo<40){lo=0;hi=255}
    for(let i=0;i<d.length;i+=4){let g=grayAt(d,i);g=Math.max(0,Math.min(255,(g-lo)*255/Math.max(1,hi-lo)));if(mode==='bw')g=g<190?0:255;else g=Math.max(0,Math.min(255,(g-128)*1.28+128));d[i]=d[i+1]=d[i+2]=g;d[i+3]=255}
    x.putImageData(im,0,0);return c;
  }

  function scoreText(text){
    const t=String(text||'').toUpperCase();
    const words=['PART NO','PART NUMBER','LOT NO','QTY','QUANTITY','DATE','ASSY','SHAPE','MLOT','BIN','MC','VC','P1','P2','GP','SERIAL','S/N','MODEL'];
    let s=Math.min(70,t.replace(/\s/g,'').length/4);words.forEach(w=>{if(t.includes(w))s+=24});
    s+=(t.match(/\([0-9A-Z]{1,5}\)/g)||[]).length*7;s+=Math.min(36,(t.match(/[:：]/g)||[]).length*4);return s;
  }
  ...
  async function recognize(worker,canvas,psm='6'){
    ...
    const r=await worker.recognize(canvas);return {text:String(r?.data?.text||'').trim(),confidence:Number(r?.data?.confidence||0)};
  }

  async function chooseOrientation(worker,canvas,onProgress){
    const preview=enhanceCanvas(canvas,'gray'),order=[0,90,270,180],tested=[];
    for(let i=0;i<order.length;i++){
      const deg=order[i];onProgress?.(`正在判斷原稿方向… ${i+1}/${order.length}`);
      const c=rotateCanvas(preview,deg),r=await recognize(worker,c,'6'),score=scoreText(r.text)+(r.confidence||0)*.25;
      tested.push({deg,score,text:r.text});if(score>=220&&r.text.length>=90)break;
    }
    tested.sort((a,b)=>b.score-a.score);const best=tested[0]||{deg:0};
    return{deg:best.deg,canvas:rotateCanvas(canvas,best.deg)};
  }
  ...
  function detectLabelBands(canvas){
    const x=canvas.getContext('2d',{willReadFrequently:true}),im=x.getImageData(0,0,canvas.width,canvas.height),d=im.data;
    const step=Math.max(1,Math.ceil(canvas.width/1500)),counts=new Uint32Array(canvas.height),sampled=Math.ceil(canvas.width/step);
    ...
    const threshold=Math.max(5,Math.round(sampled*.005));
    let rs=mergeRuns(runs([...counts].map(n=>n>threshold)),Math.max(12,Math.round(canvas.height*.022)));
    rs=rs.filter(r=>r[1]-r[0]>=Math.max(50,canvas.height*.045));
    if(rs.length<2||rs.length>8)rs=[[0,canvas.height-1]];
    return rs.map(([a,b])=>{
      const py=Math.round(canvas.height*.015),y=Math.max(0,a-py),y2=Math.min(canvas.height,b+py),rough=crop(canvas,0,y,canvas.width,y2-y),cb=contentBounds(rough,238);
      return{x:cb.x,y:y+cb.y,w:cb.w,h:cb.h};
    });
  }

  function cleanLine(v){return String(v||'').replace(/[\u2018\u2019]/g,"'").replace(/[\u201C\u201D]/g,'"').replace(/[|¦]+/g,' ').replace(/\s+/g,' ').trim()}
  function cleanValue(v){
    let s=cleanLine(v).replace(/^[\s:：=._-]+/,'').replace(/[\s|¦]+$/,'');
    s=s.replace(/\s+(?:ROHS|COMPLIANT|HF|PB)\b.*$/i,'').trim();
  ...
  function findAnchors(line){
    ...
    for(const def of FIELD_DEFS){
      for(const alias of def.aliases){
        const rx=new RegExp(aliasPattern(alias),'ig');let m;while((m=rx.exec(line)))hits.push({start:m.index,end:rx.lastIndex,def});
      }
      if(def.code){
        const code=def.code.replace(/1/g,'[1IL]').replace(/0/g,'[0O]');
        const rx=new RegExp(`[\\(\\[]?\\s*${code}\\s*[\\)\\]]?\\s*(?=${def.aliases.map(aliasPattern).join('|')})`,'ig');let m;while((m=rx.exec(line)))hits.push({start:m.index,end:rx.lastIndex,def,codeOnly:true});
      }
    }
    hits.sort((a,b)=>a.start-b.start||b.end-a.end);
    const out=[];for(const h of hits){if(out.some(x=>h.start>=x.start&&h.end<=x.end))continue;out.push(h)}return out.sort((a,b)=>a.start-b.start);
  }

  function parseKnownAnchors(line){
    const anchors=findAnchors(line).filter(a=>!a.codeOnly);if(!anchors.length)return[];const out=[];
    for(let i=0;i<anchors.length;i++){
      const a=anchors[i],next=anchors[i+1],before=line.slice(Math.max(0,a.start-9),a.start),codeMatch=before.match(/[\(\[]?\s*([0-9ILO]{0,2}[A-Z])\s*[\)\]]?\s*$/i);
      let code=codeMatch?codeMatch[1].toUpperCase().replace(/^I(?=[A-Z])$/,'1').replace(/^L(?=[A-Z])$/,'1'):a.def.code;
      const value=cleanValue(line.slice(a.end,next?next.start:line.length));if(!value||!/\w|[+\-]/.test(value))continue;
      out.push({code:code||a.def.code||'',name:a.def.name,value});
    }
    return out;
  }

  function parseCodeChunks(line){
    const codeRx=/[\(\[]?\s*((?:1P|1T|30P|31P|Q|10D|21L|16D|31T|33P|23L|24L|1Y|2Y|4Y))\s*[\)\]]?/ig;
    const ms=[...line.matchAll(codeRx)];if(!ms.length)return[];const out=[];
    for(let i=0;i<ms.length;i++){
      const m=ms[i],code=m[1].toUpperCase(),end=i+1<ms.length?ms[i+1].index:line.length,chunk=cleanLine(line.slice((m.index||0)+m[0].length,end));
      const name=CODE_MAP[code]||'',aliases=(FIELD_DEFS.find(d=>d.code===code)?.aliases||[]).sort((a,b)=>b.length-a.length);
      let rest=chunk;for(const a of aliases){rest=rest.replace(new RegExp('^\\s*'+aliasPattern(a)+'\\s*[:：=]?\\s*','i'),'')}
      rest=rest.replace(/^\s*[^:：]{0,30}[:：]\s*/,'');const value=cleanValue(rest);if(value)out.push({code,name:name||code,value});
    }
    return out;
  }
  ...
  function parseFields(text){
    ...
    for(const raw of String(text||'').split(/\r?\n/)){
      ...
      const known=parseKnownAnchors(line);out.push(...known);
      if(!known.length)out.push(...parseCodeChunks(line));
      if(!known.length){
        const m=line.match(/^([A-Z][A-Z0-9 ._/#\-]{1,38}?)\s*[:：=]\s*(.{1,120})$/i);
        if(m){const name=cleanLine(m[1]),value=cleanValue(m[2]);if(value&&/[A-Z0-9]/i.test(value))out.push({code:'',name,value})}
      }
    }
  ...
  function aggregateFields(passes){
    ...
    for(const pass of passes){for(const f of parseFields(pass.text)){const key=fieldKey(f);if(!key)continue;if(!buckets.has(key))buckets.set(key,[]);buckets.get(key).push({...f,passConfidence:pass.confidence||0})}}
    const out=[];
    for(const rows of buckets.values()){
      const variants=new Map();for(const r of rows){const k=norm(r.value);if(!k)continue;if(!variants.has(k))variants.set(k,{count:0,confidence:0,rows:[]});const v=variants.get(k);v.count++;v.confidence+=r.passConfidence||0;v.rows.push(r)}
      const ranked=[...variants.entries()].sort((a,b)=>b[1].count-a[1].count||b[1].confidence-a[1].confidence);if(!ranked.length)continue;
      const [bestKey,bestMeta]=ranked[0],base=bestMeta.rows.sort((a,b)=>(b.passConfidence||0)-(a.passConfidence||0))[0];
      out.push({code:base.code,name:base.name,value:base.value,repeat:bestMeta.count,conflict:ranked.length>1,alternatives:ranked.slice(1,3).map(x=>x[1].rows[0].value)});
    }
    return out;
  }
  ...
  async function scanRegionDeep(canvas,labelNo,onProgress){
    ...
    const c=trimCanvas(canvas),candidates=[{c,name:`標籤 ${labelNo} 全圖`}],h=c.height*.30;
    for(let i=0;i<6;i++){const y=Math.min(c.height-h,i*c.height*.14);candidates.push({c:crop(c,0,y,c.width,h),name:`標籤 ${labelNo} 區域 ${i+1}`})}
    const all=[];for(let i=0;i<candidates.length;i++){onProgress?.(`正在讀取標籤 ${labelNo} 的條碼… ${i+1}/${candidates.length}`);try{all.push(...await core.scanCanvas(candidates[i].c,candidates[i].name))}catch(e){console.warn('[LW interpreter barcode]',e)}}
  ...
  function makeTiles(canvas){
    const c=enhanceCanvas(canvas,'gray'),out=[],overlap=.08;
    for(let ry=0;ry<2;ry++)for(let rx=0;rx<2;rx++){
      const x0=Math.max(0,(rx*.5-overlap)*c.width),y0=Math.max(0,(ry*.5-overlap)*c.height),x1=Math.min(c.width,((rx+1)*.5+overlap)*c.width),y1=Math.min(c.height,((ry+1)*.5+overlap)*c.height);
      out.push(crop(c,x0,y0,x1-x0,y1-y0));
    }
    return out;
  }
  ...
  async function readRegionFields(worker,region,onProgress){
    const passes=[];
    onProgress?.('正在整理欄位內容…');
    const gray=enhanceCanvas(region,'gray'),bw=enhanceCanvas(region,'bw');
    passes.push(await recognize(worker,gray,'6'));passes.push(await recognize(worker,bw,'6'));
    let fields=aggregateFields(passes);
    if(fields.length<6||fields.some(f=>f.conflict)){
      const tiles=makeTiles(region);for(let i=0;i<tiles.length;i++){onProgress?.(`正在補讀細小欄位… ${i+1}/${tiles.length}`);passes.push(await recognize(worker,tiles[i],'6'))}
  ...
  async function processCanvas(canvas,sourceName,pageNo,worker,labels,onProgress){
    ...
    onProgress?.(`找到 ${bands.length} 個標籤區域，正在逐張讀取…`);
    for(let i=0;i<bands.length;i++){
      const b=bands[i],region=crop(best.canvas,b.x,b.y,b.w,b.h),read=await readRegionFields(worker,region,onProgress),barcodes=await scanRegionDeep(region,labels.length+1,onProgress);
      const allText=read.passes.map(p=>p.text).join('\n'),marks=detectMarks(allText);
      labels.push({sourceName,page:pageNo,index:i+1,rotation:best.deg,fields:read.fields,barcodes,marks});
    }
  }
  ...
  async function interpretPdf(file,onProgress,sharedWorker){
    ...
    try{for(let p=1;p<=pageLimit;p++){onProgress?.(`正在讀取 ${file.name} 第 ${p}/${pageLimit} 頁…`);const page=await pdf.getPage(p),canvas=await renderPage(page);await processCanvas(canvas,file.name,p,worker,labels,onProgress)}}finally{if(own)try{await worker.terminate()}catch{}}
    return{pdfPages:pdf.numPages,labels};
  }

  async function interpretImage(file,onProgress,sharedWorker){
    const own=!sharedWorker,worker=sharedWorker||await createWorker(onProgress),labels=[];try{onProgress?.(`正在讀取圖片 ${file.name}…`);await processCanvas(await imageCanvas(file),file.name,1,worker,labels,onProgress)}finally{if(own)try{await worker.terminate()}catch{}}return{imageCount:1,labels};
  }

  async function interpretFiles(files,onProgress){
    const arr=[...files].filter(f=>ext(f)==='pdf'||isImage(f)).slice(0,4),labels=[];if(!arr.length)return{files:0,pages:0,labels:[]};
    const worker=await createWorker(onProgress);let pages=0;try{for(let i=0;i<arr.length;i++){const f=arr[i];onProgress?.(`正在分析 ${i+1}/${arr.length}：${f.name}`);if(ext(f)==='pdf'){const r=await interpretPdf(f,onProgress,worker);pages+=r.pdfPages;labels.push(...r.labels)}else{const r=await interpretImage(f,onProgress,worker);pages+=1;labels.push(...r.labels)}}}finally{try{await worker.terminate()}catch{}}
  ...
  function renderInterpretation(files,result){
    ...
    const state=t.fields||t.barcodes?(t.pending?'已讀出內容，部分待確認':'主要內容已讀出'):'需要補資料';
    let html=`<div class="analysis-block"><div class="section-title"><div><h3>分析完成</h3><p class="muted compact">${esc(names)}</p></div><span class="pill">${state}</span></div>`;
    html+=`<div class="file-chips"><span class="file-chip">標籤 ${labels.length} 張</span><span class="file-chip">欄位 ${t.fields} 個</span><span class="file-chip">條碼 ${t.barcodes} 個</span><span class="file-chip">可信 ${t.confirmed} 個</span>${t.pending?`<span class="file-chip">待確認 ${t.pending} 個</span>`:''}</div>`;
  ...
  async function analyze(files){
    const arr=[...files],out=el('analysisResult');if(!out)return;const eligible=arr.filter(f=>ext(f)==='pdf'||isImage(f));
    if(!eligible.length||eligible.length!==arr.length){const base=window.LabelWorkbenchParsers;if(base?.analyze)return base.analyze(arr);throw new Error('文件解析器尚未載入')}
    lastFiles=eligible;const progress=msg=>{out.innerHTML=`<div class="scan-working"><b>正在完整讀取客戶原稿</b><br>${esc(msg)}<br><small>系統會自動分標籤、補讀小字並交叉比對條碼。</small></div>`};
    progress('準備分析…');lastResult=await interpretFiles(eligible,progress);out.innerHTML=renderInterpretation(eligible,lastResult);wireResultButtons();return lastResult;
  }
