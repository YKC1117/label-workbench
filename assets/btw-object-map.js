...
(function(){
  ...
  function ownerFor(tags,offset,maxDistance=64){let hit=null;for(const t of tags){if(t.offset<=offset)hit=t;else break}return hit&&offset-hit.offset<=maxDistance?hit.type:''}
  ...
  function barcodeComponentEntries(strings){const out=[];for(let i=0;i<strings.length-1;i++){if(strings[i].text!==PLACEHOLDER)continue;const entry=strings[i+1];if(!entry)continue;const v=String(entry.text??'').trim();if(v&&/^(?:文字範例|Box Options|DataSource|Text \d+)$/i.test(v))continue;out.push({value:v,entry:compactEntry(entry)})}return out}
  ...
  function renderDecoded(file,decoded){const h=decoded.parsed.header||{},objects=decoded.map.objects,counts={};for(const o of objects)counts[typeLabel(o)]=(counts[typeLabel(o)]||0)+1;const chips=Object.entries(counts).map(([k,n])=>`<span class="file-chip">${esc(k)} ${n}</span>`).join('');const size=/<TemplateSize>([^<]+)<\/TemplateSize>/i.exec(h.text||'')?.[1]||'';const rows=objects.slice(0,100).map(o=>`<tr><td>${esc(typeLabel(o))}</td><td>${esc(o.name||o.id)}</td><td>${o.xMm==null?'—':`${o.xMm} / ${o.yMm} mm`}</td><td>${esc(objectSummary(o))}</td><td>${esc(o.fontName||'—')}${o.fontSize!=null?` ${o.fontSize} pt`:''}</td></tr>`).join('');return `<div class="analysis-block lw-btw-decoded"><div class="section-title"><b>BTW 原生物件解析：${esc(file.name)}</b><span class="pill">${objects.length} 個物件區段</span></div><div class="file-chips"><span class="file-chip">${esc(h.applicationVersion||'BarTender')}</span>${h.compatibleVersion?`<span class="file-chip">相容 ${esc(h.compatibleVersion)}</span>`:''}${size?`<span class="file-chip">${esc(size)}</span>`:''}${chips}</div><div class="note"><b>已直接讀取 BTW：</b>位置、文字、字型/字級與條碼資料來源都來自檔案原生 serialized container，不是 OCR。Data Matrix 單一 payload 與 Code 128 多段 datasource 已具備安全寫回底層；尺寸欄位仍待 BarTender 實機驗證後才開放正式寫入。</div><div class="table-scroll"><table class="analysis-table"><thead><tr><th>類型</th><th>物件</th><th>X / Y</th><th>內容 / 資料組成</th><th>字型</th></tr></thead><tbody>${rows}</tbody></table></div>${objects.length>100?'<div class="footer-note">物件超過 100 個，畫面先顯示前 100 個。</div>':''}</div>`}
  async function waitBaseAnalysis(out,timeout=120000){const start=Date.now();while(Date.now()-start<timeout){const text=out?.textContent||'';if(!out?.querySelector('.scan-working')&&!/持續解析中|本機解析中/.test(text))return;await new Promise(r=>setTimeout(r,120))}}
  async function analyzeBtwFiles(files){const list=[...(files||[])].filter(f=>/\.btw$/i.test(f.name)).slice(0,4);if(!list.length)return;const out=document.getElementById('analysisResult');if(!out)return;const results=await Promise.all(list.map(async file=>{try{return{file,decoded:await decodeBtw(await file.arrayBuffer())}}catch(error){return{file,error}}}));await waitBaseAnalysis(out);[...out.querySelectorAll('.analysis-block')].forEach(n=>{const t=n.textContent||'';if(/BarTender \.btw/i.test(t)&&/不會在外部瀏覽器假裝解析|正式內容仍回公司/.test(t))n.remove()});out.querySelectorAll('.lw-btw-decoded,.lw-btw-error').forEach(n=>n.remove());for(const r of results){if(r.decoded)out.insertAdjacentHTML('beforeend',renderDecoded(r.file,r.decoded));else out.insertAdjacentHTML('beforeend',`<div class="analysis-block lw-btw-error"><b>BTW：${esc(r.file.name)}</b><div class="note warn-note">原生物件解析失敗：${esc(r.error?.message||r.error)}</div></div>`)}}
