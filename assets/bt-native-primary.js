/* Label Workbench editable BTW primary handoff v1.0
...
(function(){
  ...
  const BUILD='20260910-btnp100';
  const FORMAT_SRC='assets/btw-format.js?v=20260910-btw011';
  const NATIVE_SRC='assets/btw-native.js?v=20260910-btwn100';
  ...
  async function downloadEditable(){
    ...
    const buttons=[el('analysisBtNative'),el('btNativeDownload')].filter(Boolean),labels=b.latestResult.labels||[];
    buttons.forEach(x=>{x.disabled=true;x.dataset.oldText=x.textContent;x.textContent='正在建立可編輯 BTW…'});
    try{
      const api=await ensureNative(),out=await api.downloadFromAnalysis(b.latestResult,b.latestFiles,msg=>buttons.forEach(x=>x.textContent=msg||'正在建立可編輯 BTW…'));
      if(out?.ok)toast(out.count>1?`已建立 ${out.count} 個可編輯 BTW`:'可編輯 BTW 已下載');return !!out?.ok
  ...
  function decorateAnalysis(){
    ...
    hint.innerHTML='<b>主要流程：</b>下載 .btw 後直接用 BarTender 開啟；文字與支援的 Code 128／Data Matrix 會保留成可編輯物件。第一次給客戶前請確認尺寸與版面。';
  ...
  function decorateBt(){
    ...
    const intro=title?.nextElementSibling;if(intro)intro.textContent='PDF／圖片快速分析後，直接下載可編輯 .btw；回公司用 BarTender 開啟後微調尺寸、字型與位置即可。';
    ...
    const note=panels?.[2]?.querySelector('.note');if(note)note.innerHTML='<b>目前原生 BTW：</b>優先處理文字、Code 128、Data Matrix；PNG 匯入與資料包保留當備用。';
  }

  function syncHeader(){const active=document.querySelector('.view.active')?.id;if(active!=='bartender')return;const t=el('pageTitle'),s=el('pageSub');if(t)t.textContent='BT 快速製作';if(s)s.textContent='快速分析後直接產生可編輯 .btw，再到 BarTender 微調尺寸與版面。'}
