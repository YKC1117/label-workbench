...
(function(){
  ...
  async function ensureNative(){
    ...
    nativePromise=(async()=>{await ensureFormat();return loadScript(NATIVE_SRC,()=>window.LabelWorkbenchBtwNative,'BTW native')})().finally(()=>{nativePromise=null});
  ...
  async function downloadEditable(){
    ...
    try{
      ...
      toast('可編輯 BTW 建立停止：'+(err?.message||err));
      return false;
    }finally{
