...
(function(){
  ...
  const BUILD='20260918-btw-family-native-110-qr-c39-upca';
  const OFF=50000;
  ...
  function upcaSafe(value){
    const s=String(value||'').trim();if(!/^\d{12}$/.test(s))return false;
    const d=[...s].map(Number),sum=(d[0]+d[2]+d[4]+d[6]+d[8]+d[10])*3+(d[1]+d[3]+d[5]+d[7]+d[9]),check=(10-(sum%10))%10;
    return check===d[11]
  }
  function plan(label){
  ...
  window.LabelWorkbenchBtwFamilyNative={
    BUILD,PROFILES,kindFor,textItems,code39Safe,upcaSafe,plan,canGenerate,targetSize,printableTexts,adjacentSizePairs,rewriteInternalSize,retailMirrorEntry,applyRetailPayload,seedEndpoint,fetchSeed,generateOne
  };
})();