...
(function(){
  'use strict';
  const BUILD='20260912-view-guard-120-btw-captions';
  const RELEASE='v1.9.35';
  const UPDATED='2026/09/12 00:15';
  ...
  function scheduleEnforce(){if(queued)return;queued=true;requestAnimationFrame(enforce)}
  function remember(id){if(!validView(id))return;desiredView=id;scheduleEnforce();setTimeout(enforce,80);setTimeout(enforce,300)}
  function wrapShowView(){const base=window.showView;if(typeof base!=='function'||base.__lwViewGuardWrapped)return false;function guardedShowView(id){if(validView(id))desiredView=id;const result=base.apply(this,arguments);scheduleEnforce();return result}guardedShowView.__lwViewGuardWrapped=true;window.showView=guardedShowView;return true}
  document.addEventListener('click',function(event){const btn=event.target.closest?.('[data-view]');if(btn?.dataset?.view)remember(btn.dataset.view)},true);
  const observer=new MutationObserver(function(mutations){if(applying)return;if(mutations.some(m=>m.type==='attributes'&&m.attributeName==='class'))scheduleEnforce()});
  observer.observe(document.documentElement,{subtree:true,attributes:true,attributeFilter:['class']});
  stampRelease();
  if(!wrapShowView()){let tries=0;const timer=setInterval(function(){tries++;if(wrapShowView()||tries>100)clearInterval(timer)},50)}
