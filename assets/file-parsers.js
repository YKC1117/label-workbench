/* Label Workbench local document parsers. No customer file content is uploaded by this module. */
(function(){
  'use strict';
  ...
  function loadScript(src,globalName){
    ...
    const p=new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.async=true;s.onload=()=>globalThis[globalName]?resolve(globalThis[globalName]):reject(new Error(`${globalName} 未載入`));s.onerror=()=>reject(new Error(`無法載入解析元件：${src}`));document.head.appendChild(s)});
  ...
  async function parsePdf(file){
    ...
    const pages=Math.min(pdf.numPages,12),chunks=[];
    for(let p=1;p<=pages;p++){const page=await pdf.getPage(p),content=await page.getTextContent();chunks.push(content.items.map(x=>x.str||'').join(' '))}
    const text=chunks.join('\n').trim(),keys=labelKeywords(text),preview=text.slice(0,5000);
    return {text,html:`<div class="analysis-block"><b>PDF：${esc(file.name)}</b><div class="file-chips"><span class="file-chip">共 ${pdf.numPages} 頁</span><span class="file-chip">解析前 ${pages} 頁</span><span class="file-chip">文字 ${text.length} 字元</span></div>${keys.length?`<div class="footer-note"><b>原稿文字中出現：</b>${keys.map(esc).join('、')}</div>`:''}${preview?`<div class="document-preview">${esc(preview)}</div>`:'<div class="note warn-note">沒有擷取到文字。這份 PDF 可能是掃描圖片，後續需 OCR / AI 視覺分析，這裡不會亂猜。</div>'}${text.length>preview.length?'<div class="footer-note">預覽只顯示前 5,000 字。</div>':''}</div>`}
  }

  async function parseOne(file){const e=ext(file);if(['xls','xlsx'].includes(e))return parseExcel(file);if(e==='csv')return parseCsv(file);if(['doc','docx'].includes(e))return parseDocx(file);if(e==='pdf')return parsePdf(file);return null}
  ...
  async function analyze(files){
    ...
    html+=`<div class="analysis-block"><b>製作前仍要確認：</b><br>• Label 實際尺寸（寬 × 高）<br>• 印表機品牌 / 型號 / DPI<br>• 固定欄位與每張變動欄位<br>• 條碼種類、真正掃描內容、前後綴與 GS1 規則<br>• Excel / CSV 每列資料是否就是一張標籤</div><div class="note"><b>隱私：</b>這一段 PDF / Excel / Word / CSV 解析在你的瀏覽器本機執行。解析套件從公開 CDN 載入，但客戶檔案內容不會送到 CDN。</div>`;
    out.innerHTML=html;
  }

  function updateUi(){
    const small=document.querySelector('.brand small');if(small&&/v0\.\d+/i.test(small.textContent||''))small.textContent=(small.textContent||'').replace(/v0\.\d+/i,'v0.8');
    const note=[...(el('analysis')?.querySelectorAll('.warn-note')||[])].find(n=>(n.textContent||'').includes('目前不會亂猜'));
    if(note)note.innerHTML='<b>已啟用本機解析：</b>Excel / CSV 可讀工作表與欄位、DOCX 可擷取文字、PDF 可擷取文字層。掃描型 PDF / 圖片 OCR、圖片條碼辨識與 .btw 物件解析仍不會亂猜。';
    const quick=[...(el('dashboard')?.querySelectorAll('.quick button')||[])].find(b=>(b.textContent||'').includes('客戶原稿快速分析'));
    const span=quick?.querySelector('span');if(span)span.textContent='本機讀取 PDF / Excel / Word / CSV 與圖片基本資訊';
  }
  ...
  window.LabelWorkbenchParsers={parseCsvLine,labelKeywords,classify,parseExcel,parseCsv,parseDocx,parsePdf,analyze};
})();
